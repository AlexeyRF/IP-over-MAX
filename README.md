# IP-over-MAX (v1)


Протокол передачи данный, использующий мессенджер MAX.

## Основные возможности

*   **Четыре режима передачи данных**:
    *   **PTCP (Pseudo-TCP)** [Port 10001]: Гарантированная доставка. Проверяет историю чата на наличие сообщения и переотправляет его каждые 15 секунд до подтверждения появления.
    *   **PUDP (Pseudo-UDP)** [Port 10002]: Ждет активного соединения с сервером перед отправкой пакета, но не проверяет историю.
    *   **UDP (True-UDP)** [Port 10003]: Отправляет сообщение немедленно, без проверок.
    *   **CUDP (Clean-UDP)** [Port 10005]: Сообщения автоматически удаляются из чата через 60 секунд после отправки.
*   **Шифрование и типы пакетов**:
    *   Автоматическая генерация RSA-2048 ключей и UUID при первом запуске.
    *   Шифрование контента (OAEP SHA256) для персональных сообщений.
    *   Открытая передача для широковещательных (`broadcast`) пакетов.
*   **Автоматическая фрагментация**: Разбиение длинных данных (более 3000 символов) на части и их сборка на стороне получателя.
*   **Локальный шлюз**: Работает как прозрачный прокси, принимающий и отправляющий данные через локальные UDP-порты.

## Быстрый старт

### 1. Установка зависимостей

```bash
pip install maxapi-python cryptography aiohttp
```

### 2. Конфигурация
Отредактируйте переменные в начале файла `ip-over-max.py`:

```python
CHAT_ID = 0            # ID чата для обмена данными (0 - избранное)
```

Вход реализован по QR, поэтому номер телефона менять не надо.

### 3. Запуск клиента
```bash
python ip-over-max.py
```

## Работа с портами (Локальный API)

Клиент слушает локальные порты и пересылает данные в MAX.

| Порт | Режим | Описание |
| :--- | :--- | :--- |
| **10001** | **PTCP** | Гарантированная доставка с проверкой истории |
| **10002** | **PUDP** | Отложенная отправка до появления сети |
| **10003** | **UDP** | Обычная быстрая отправка |
| **10005** | **CUDP** | Самоуничтожение сообщения через 60 секунд |
| **10004** | **RECEIVE** | Порт, на который клиент пересылает входящие данные |

### Формат отправки (UDP на локальный порт)
Вы можете отправлять данные в двух форматах:
1.  **Broadcast**: `Любой текст сообщения` — увидят все участники.
2.  **Targeted**: `BASE64_UUID|Текст сообщения` — будет зашифровано для конкретного получателя.

## Примеры использования (`examples_send.py`)

```python
import socket

# Отправка сообщения с самоликвидацией через 60 секунд
def send_secure_iot_data():
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        message = "Данные датчика: 22.5C"
        sock.sendto(message.encode(), ("127.0.0.1", 10005)) # CUDP

# Отправка зашифрованного сообщения конкретному устройству
def send_to_device(device_uuid_b64, text):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        payload = f"{device_uuid_b64}|{text}"
        sock.sendto(payload.encode(), ("127.0.0.1", 10001)) # PTCP
```

## Технические подробности

### Структура пакета в MAX
Все данные передаются в формате JSON:

```json
{
  "protocol": "IP-over-MAX-v1", // Программа отбрасывает все пакеты имеющие другой протокол 
  "type": "content", // или "start", "repeat_start", "content_part"
  "author": "BASE64_UUID",
  "recipient": "broadcast", // или "BASE64_UUID"
  "content": "BASE64_ENCRYPTED_DATA",
  "part": 1,          // (optional) для фрагментации
  "total_parts": 2    // (optional) для фрагментации
}
```

### Хранение данных
*   `cache/identity.json`: Ваш UUID и приватный ключ. **Никому не передавайте этот файл.**
*   `cache/known_uuids.json`: Справочник публичных ключей других участников сети.
*   `cache/session.db`: Сессия подключения к MAX. **Никому не передавайте этот файл.**

### Планы на IP-over-MAX v2 
Добавить использование файлов (IP over zdisk режим) вместо деления сообщения на части. Для вас, как для пользователей протокола, ничего не поменяется, результат будет тот же.

